...
void guardarHistorial() {
    ...
    for (int i = 0; i < numeroJugadas; ++i) {
        fprintf(archivo, "%d %d %d %d %ld\n", historialJugadas[i].tipo, historialJugadas[i].numeros[0], historialJugadas[i].numeros[1], historialJugadas[i].numeros[2], historialJugadas[i].hora);
    }
...
void cargarHistorial() {
    ...
    while (fscanf(archivo, "%d %d %d %d %ld", &historialJugadas[numeroJugadas].tipo, &historialJugadas[numeroJugadas].numeros[0], &historialJugadas[numeroJugadas].numeros[1], &historialJugadas[numeroJugadas].numeros[2], &historialJugadas[numeroJugadas].hora) == 5) {
        numeroJugadas++;
...
void imprimirResultado(int numerosGanadores[3]) {
    ...
    if (coincidencias > 0) {
        ...
        printf("Lo siento, no has ganado esta vez. ¡Mejor suerte la próxima hijo de tu matre !\n");
    }
}
...
void jugar() {
    ...
    if (historialJugadas[numeroJugadas].tipo == PALE) {
        // Para Pale, permitir al usuario ingresar solo dos números
        for (int i = 0; i < 2; ++i) {
            printf("Ingrese el número %d: ", i + 1);
            scanf("%d", &historialJugadas[numeroJugadas].numeros[i]);
        }
        // Establecer el tercer número en cero para indicar que no se usa en esta jugada
        historialJugadas[numeroJugadas].numeros[2] = 0;
    } else {
